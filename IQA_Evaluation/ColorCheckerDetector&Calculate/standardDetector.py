# 修改 main 函数中参数，运行可自动识别大部分情况下，在图片内存在的 Macbeth ColorChecker，并自动裁剪旋转至 6*4 状态展示
# 但在色卡被遮挡情况下可能识别不出
import cv2
...
for i in range(0,4):
    ...
    # print(f"Rotation {i}: shape = {numpy.rot90(labStandardColors,-i).shape}")
labRotated = numpy.array(labRotated, dtype=object)
class Square:
    def __init__(self):
        pass
...
def findStandard(img, downSample=True):
    ...
    for r, row in enumerate(standardColors):
        for c, color in enumerate(row):           
            ...
            print(iNow,': Calculating color ', labColor)
            iNow=iNow+1
    ...
    # print("length:____",len(horizontalOffsets), len(verticalOffsets), len(squares))
...
if __name__ == '__main__':
    import os
    # directory = './checker_input'
    directory = './New4_2_C1'
    files = [os.path.join(directory, file) for file in os.listdir(directory) if file[-5:] == '.jpeg']
    for file in files:
        print(file)
