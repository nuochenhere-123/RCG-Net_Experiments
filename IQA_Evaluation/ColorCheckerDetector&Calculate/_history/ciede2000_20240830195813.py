...
def main():
    ...
    # 需要计算指标的图片所在路径
    dir = './New4_2_C1'
    result_dirs = os.listdir(dir)
    # 处理不同图像的指标值
    for imgdir in result_dirs:
        sumImage = 0.0 # 一个图像内 24个色块 的 CIEDE2000 均值之和
        colorSum = 0.0
        # 针对某个图像计算不同色块的指标值，并保留所有色块的均值
        if  imgdir.endswith('_out.png') or imgdir.endswith('_out.jpg') or imgdir.endswith('_out.jpeg'): # 文件后缀
          # 1: dark skin
            colorSum = colorSum+1 # 处理第一个色块
            print(colorSum)
            # 随机选取 10 个点
        ...
        #     sumImage += CIEDE2
        avgImage = sumImage/colorSum
        sumCIEDE += avgImage
        print(imgdir, "中24个色块的 CIEDE2000 均值 为: ", avgImage)
    
    print("checker内所有图像 的 CIEDE2000 均值 为: ", sumCIEDE)
