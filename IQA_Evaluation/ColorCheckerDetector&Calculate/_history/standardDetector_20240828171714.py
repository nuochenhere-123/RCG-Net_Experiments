...
def findStandard(img, downSample=True):
    ...
    if HIGH_MEMORY:
        ...
        if cv2.waitKey(0) == ord('q'):
            quit()
            '''
        nMap = p.map(lambda args: calcDistance(*args), [(c, labimg.shape) for c in labStandardColors.reshape((labStandardColors.shape[0]*labStandardColors.shape[1], labStandardColors.shape[2]))])    
