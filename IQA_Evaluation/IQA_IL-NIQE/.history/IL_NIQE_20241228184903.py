# 源代码相关：https://github.com/IceClear/IL-NIQE
# 运行： python IL_NIQE.py .\result_dir
# eg:   python IL_NIQE.py .\upgrade7_5-U45
#       python IL_NIQE.py ./RetinexBased-U45   
# 每次运行修改：1. 处理图片的命名格式   2. 重命名txt文件名
...
import time
# import ray
from IL_NIQE_matlab_resize import MATLABLikeResize
...
if __name__ == '__main__':
    ...
    for imgdir in result_dirs:
        ...
        # if  imgdir.endswith('_out.png') or imgdir.endswith('_out.jpg') or imgdir.endswith('_out.jpeg'): # 文件后缀
            img = cv2.imread(os.path.join(result_path,imgdir))
            #图片名称为imgdir，此处提取前缀
            ...
            with warnings.catch_warnings():
                ...
                #第几张：图片名称
                print(imagenum," - ",imgname_cor," NIQE: ", niqe_result, " with dealing time: ", time_used)
                # 更新数值
                imagenum=imagenum+1
                sumNIQE += niqe_result
                sumTime += time_used
                ...
                with open(os.path.join(result_path, save_txt_name), 'a') as f:
                    f.write('{}: NIQE={}  with dealing time: {}\n'.format(imgdir, niqe_result, time_used))
                  
    # 记录图片结果均值
    mNIQE = sumNIQE/N
    mTime = sumTime/N
    # 将 指标 数值转换为字符串格式，保留六位小数
    mNIQE_str = "{:.6f}".format(mNIQE)
    with open(os.path.join(result_path, save_txt_name), 'a') as f:
        f.write('\n{} Image Average: NIQE={}, TimePerImage={}, SumTime = {}s\n\n'.format(N, mNIQE, mTime, sumTime))
    print("Sum:{} Average: NIQE={}, TimePerImage={}, SumTime = {}s".format(N, mNIQE, mTime, sumTime))
    
    
    # 重命名文件名
    # save_txt_name_new = '000_NIQE-{}_Orginal.txt'.format(mNIQE_str)
    # save_txt_name_new = '000_NIQE-{}_epoch50.txt'.format(mNIQE_str)
    # save_txt_name_new = 'New4_4_C1_4-256-ep137-NIQE-{}.txt'.format(mNIQE_str)
    # save_txt_name_new = 'WO-SCA-OceanDark-ep137-NIQE-{}.txt'.format(mNIQE_str)
    # save_txt_name_new = 'C60-PUGAN-Pretrain NIQE-{}.txt'.format(mNIQE_str)  # 32resize & 256To256  C60-resize
    # save_txt_name_new = 'C60-resize-FUnIE-ReTrain NIQE-{}.txt'.format(mNIQE_str)  # 32resize & 256To256  C60-resize
    # save_txt_name_new = 'UCCS-DICAM-ReTrain NIQE-{}.txt'.format(mNIQE_str)
    # save_txt_name_new = 'C60-resize-U_Shape_Transformer-ReTrain NIQE-{}.txt'.format(mNIQE_str)
    # save_txt_name_new = 'UIQS-WaterNet-ReTrain NIQE-{}.txt'.format(mNIQE_str)
    # save_txt_name_new = 'UIQS-256-Ucolor-Pretrain NIQE-{}.txt'.format(mNIQE_str)
    # save_txt_name_new = 'U45-Fusion NIQE-{}.txt'.format(mNIQE_str)
    save_txt_name_new = 'OceanDark-RetinexBased NIQE-{}.txt'.format(mNIQE_str)
    # save_txt_name_new = 'OceanDark-256-GDCP NIQE-{}.txt'.format(mNIQE_str)
    # save_txt_name_new = 'UIQS-256-ZSRM NIQE-{}.txt'.format(mNIQE_str)
    # save_txt_name_new = 'U45-PCFB NIQE-{}.txt'.format(mNIQE_str)
    # save_txt_name_new = 'UCCS-256-UWMamba-Pretrain NIQE-{}.txt'.format(mNIQE_str)
