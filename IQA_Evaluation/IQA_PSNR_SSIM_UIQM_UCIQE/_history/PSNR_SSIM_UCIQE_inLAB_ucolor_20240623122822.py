...
# eg. python PSNR_SSIM_UCIQE_inLAB_ucolor.py .\result .\real_inference
#     python PSNR_SSIM_UCIQE_inLAB_ucolor.py .\upgrade2_3-R90 .\UIEB-reference-890
...
def save_with_IQA(result_path,corrected,imgdir,psnr_str,ssim_str,uiqm_str,uciqe_str):
        # 计算完成后标有指标数值的图片存储路径
        ex='PSNR_SSIM_UIQM_UCIQE-upgrade7_4-Test-110-' # 文件夹名称
        # ex='PSNR_SSIM_UIQM_UCIQE-upgrade7_4-R90-' # 文件夹名称
        res_folder = os.path.join(result_path, ex)
        if not os.path.exists(res_folder):
            os.makedirs(res_folder)
        # 在图片上绘制文本
        font = cv2.FONT_HERSHEY_SIMPLEX
        bottomLeftCornerOfText = (int(corrected.shape[1] * 0.8), int(corrected.shape[0] * 0.75)) # 宽*0.8处 高*0.79处
         # 根据图像宽度动态调整字体大小
...
def main():
    ...
    for imgdir in result_dirs:
        ...
        if  imgdir.endswith('_out.png'): # 文件后缀   随机应变改！！！
            ...
            imagenum += 1
            # 记录单张图片结果
            with open(os.path.join(result_path,'upgrade2_3-Test-110 PSNR & SSIM & UIQM & UCIQE.txt'), 'a') as f:
            # with open(os.path.join(result_path,'upgrade7_4-R90 PSNR & SSIM & UIQM & UIQM & UCIQE.txt'), 'a') as f:
    ...
    muciqe = sumuciqe/N
    # 记录图片结果均值
    with open(os.path.join(result_path,'upgrade2_3-Test-110 PSNR & SSIM & UCIQE.txt'), 'a') as f:
    # with open(os.path.join(result_path,'upgrade7_4-R90 PSNR & SSIM & UIQM & UCIQE.txt'), 'a') as f:
