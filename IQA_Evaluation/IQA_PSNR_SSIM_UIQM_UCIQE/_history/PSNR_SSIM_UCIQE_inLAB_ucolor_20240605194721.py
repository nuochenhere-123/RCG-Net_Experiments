...
def save_with_IQA(result_path,corrected,imgdir,psnr_str,ssim_str,uciqe_str):
        # 计算完成后标有指标数值的图片存储路径
        ex='PSNR_SSIM_UCIQE-upgrade2_3-R90-' # 文件夹名称
...
def main():
    ...
    for imgdir in result_dirs:
        ...
        if  imgdir.endswith('png_out.png'): # 文件后缀   随机应变改！！！
            #corrected image
            corrected = cv2.imread(os.path.join(result_path,imgdir))
            
            #reference image
            imgname = imgdir.split('.png_out.')[0] # 以"."分为前后两部分 随机应变改！！！
            refdir = imgname+'.'+imgdir.split('.png_out.')[1] # 参考图像名称（含后缀）随机应变改！！！
            print(refdir)
            ...
            imagenum += 1
            # 记录单张图片结果
            with open(os.path.join(result_path,'upgrade2_3-R90 PSNR & SSIM & UCIQE.txt'), 'a') as f:
