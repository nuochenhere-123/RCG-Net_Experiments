...
def save_with_IQA(result_path,corrected,imgdir,psnr_str,ssim_str,uciqe_str):
        # 计算完成后标有指标数值的图片存储路径
        # ex='PSNR_SSIM_UCIQE-upgrade7_3-Test-110-' # 文件夹名称
        ex='PSNR_SSIM_UCIQE-upgrade7_3-R90-' # 文件夹名称
...
def main():
    ...
    for imgdir in result_dirs:
        ...
        if  imgdir.endswith('_out.png'): # 文件后缀   随机应变改！！！
            ...
            imagenum += 1
            # 记录单张图片结果
            # with open(os.path.join(result_path,'upgrade7_3-Test-110 PSNR & SSIM & UCIQE.txt'), 'a') as f:
            with open(os.path.join(result_path,'upgrade7_3-R90 PSNR & SSIM & UCIQE.txt'), 'a') as f:
