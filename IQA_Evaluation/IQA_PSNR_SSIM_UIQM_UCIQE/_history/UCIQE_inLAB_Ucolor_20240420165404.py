...
eg. python UCIQE_Ucolor_inLAB.py .\RESULT_PATH
...
def main():
    ...
    for imgdir in result_dirs:
        ...
        # if  imgdir.endswith('.png_out.png'): # 文件后缀
            ...
            N +=1
            # 记录单张图片结果
            with open(os.path.join(result_path,'UCIQE_Ucolor_inLAB.txt'), 'a') as f:
                f.write('{}: uciqe={}\n'.format(imgdir,uciqe))
    ...
    with open(os.path.join(result_path,'UCIQE_Ucolor_inLAB.txt'), 'a') as f:
        f.write('\nAverage: uciqe={}\n\n'.format(muciqe))
    print("Average: uciqe={}".format(muciqe))
