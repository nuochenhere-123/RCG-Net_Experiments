...
torch.cuda.set_per_process_memory_fraction(0.9, device=0)  # 允许进程使用90%的显存
ch = 3
...
result_dir = 'results/DICAM-C60-ok/'
...
if __name__ =='__main__':
    ...
    with tqdm(total=len(total_files)) as t:
        ...
        for m in total_files:
            ...
            output = output[:, :, ::-1]
            # # resize输入，恢复为原来的大小
            # output = cv2.resize(output, original_size, interpolation=cv2.INTER_AREA)
