...
gen_dir = "./DICAM-U45" 
gen_uqims = measure_UIQMs(gen_dir)
print ("DICAM Enhanced UIQMs >> Mean: {0} std: {1}".format(np.mean(gen_uqims), np.std(gen_uqims)))

gen_dir = "./UShape-U45" 
gen_uqims = measure_UIQMs(gen_dir)
print ("UShape Enhanced UIQMs >> Mean: {0} std: {1}".format(np.mean(gen_uqims), np.std(gen_uqims)))

gen_dir = "./RES_GDCP-U45" 
gen_uqims = measure_UIQMs(gen_dir)
print ("GDCP Enhanced UIQMs >> Mean: {0} std: {1}".format(np.mean(gen_uqims), np.std(gen_uqims)))
