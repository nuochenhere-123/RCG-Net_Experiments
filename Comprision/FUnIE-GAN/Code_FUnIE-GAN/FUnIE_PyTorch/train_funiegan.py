...
samples_dir = os.path.join("samples_reTrain/FunieGAN/", dataset_name)
checkpoint_dir = os.path.join("checkpoints_reTrain/FunieGAN/", dataset_name)
...
if args.epoch == 0:
    ...
    generator.load_state_dict(torch.load("checkpoints_reTrain/FunieGAN/%s/generator_%d.pth" % (dataset_name, args.epoch)))
    discriminator.load_state_dict(torch.load("checkpoints_reTrain/FunieGAN/%s/discriminator_%d.pth" % (dataset_name, epoch)))
...
for epoch in range(epoch, num_epochs):
    for i, batch in enumerate(dataloader):
        ...
        if batches_done % val_interval == 0:
            ...
            save_image(img_sample, "samples_reTrain/FunieGAN/%s/%s.png" % (dataset_name, batches_done), nrow=5, normalize=True)

    ## Save model checkpoints
    if (epoch % (ckpt_interval*5) == 0):
        if os.path.exists(checkpoint_dir):
          shutil.rmtree(checkpoint_dir)  # 删除目录及其所有内容
          os.makedirs(checkpoint_dir)  # 重新创建目录
    if (epoch % ckpt_interval == 0):
        torch.save(generator.state_dict(), "checkpoints_reTrain/FunieGAN/%s/generator_%d.pth" % (dataset_name, epoch))
        torch.save(discriminator.state_dict(), "checkpoints_reTrain/FunieGAN/%s/discriminator_%d.pth" % (dataset_name, epoch))
