...
class GetTrainingPairs(Dataset):
    ...
    def get_file_paths(self, root, dataset_name):
        ...
        elif dataset_name=='UIEB780':
            filesA, filesB = [], []
            filesA += sorted(glob.glob(os.path.join(root, 'raw-780') + "/*.*"))
            filesB += sorted(glob.glob(os.path.join(root, 'reference-780') + "/*.*"))
...
class GetValImage(Dataset):
    ...
    def get_file_paths(self, root, dataset_name):
        if dataset_name=='EUVP':
            ...
            files = []
            files += sorted(glob.glob(os.path.join(root, 'raw-60') + "/*.*"))
        return files
