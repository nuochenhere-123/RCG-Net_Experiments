...
% 获取图片列表
input_folder = 'ImageForTest/Test-OceanDark/';
output_folder = 'result/Fusion-OceanDark/';
...
for k = 1:length(image_files)
    ...
    disp(['Processing file ' num2str(k) ' which is: ' filename]);
    % 加载图片
    img = load_image(input_folder, filename);
    ...
    imwrite(fusion, output_path);
    counter = counter + 1;
end
disp([num2str(counter), '张图片处理完成并保存到 ', output_folder, ' 文件夹中']);