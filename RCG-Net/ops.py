...
def tf_conv2d_transpose(input_tensor, filters, kernel_size=3, strides=2, padding='same', output_padding=None, use_bias=True, bias_initializer='zeros', name="deconv2d"):
  with tf.variable_scope(name):
    # 定义转置卷积层
    deconv_layer = tf.keras.layers.Conv2DTranspose(
        filters=filters, # 输出通道数
        kernel_size=kernel_size, # 卷积核大小
        strides=strides, # 卷积步长,输出张量的空间维度将是输入张量的空间维度乘以2(strides的值)
        padding=padding, # 填充方式（'same' 或 'valid'，默认值为'same'）
        output_padding=output_padding, # 输出填充（默认值为None）
        use_bias=use_bias,        # 是否使用 bias
        bias_initializer=bias_initializer  # bias 的初始化方法
    )
    # 应用转置卷积层
...
def conv2d_depthwise(input_, input_dim,output_dim, 
       k_h=5, k_w=5, d_h=2, d_w=2, stddev=0.02,
       name="conv2d"):
  with tf.variable_scope(name):
    #
    ...
    biases = tf.get_variable('biases', [input_dim], initializer=tf.constant_initializer(0.0))
    conv = tf.reshape(tf.nn.bias_add(conv, biases), conv.get_shape())
...
def Fully_connected(x, units=512, layer_name='fully_connected') :
    with tf.name_scope(layer_name) :
        return tf.layers.dense(inputs=x, use_bias=False, units=units)

def lrelu_refine(x, leak=0.2, name="lrelu"):
     with tf.variable_scope(name):
         f1 = 0.5 * (1 + leak)
         f2 = 0.5 * (1 - leak)
